...
function construction_firm_excerpt_more( $link ) {
	...
	$link = sprintf(
		'<div class="link-more text-center"><a href="%1$s" class="more-link">%2$s</a></div>',
...
function construction_firm_notice(){
    global $pagenow;
    $theme_name = wp_get_theme();
    if ( is_admin() && ('themes.php' == $pagenow) && isset( $_GET['activated'] ) && $theme_name->get( 'TextDomain' ) === 'construction-firm' ) {
   		wp_safe_redirect( admin_url("themes.php?page=construction-firm-guide-page") );
   	}
}
add_action('after_setup_theme', 'construction_firm_notice');

function construction_firm_add_new_page() {
    $edit_page = admin_url().'post-new.php?post_type=page';
    echo json_encode(['page_id'=>'','edit_page_url'=> $edit_page ]);

    exit;
}
add_action( 'wp_ajax_construction_firm_add_new_page','construction_firm_add_new_page' );
...
function construction_firm_scripts() {
	...
	wp_add_inline_style( 'construction-firm-style',$construction_firm_custom_style );
	...
	// Block Style
	wp_enqueue_style( 'construction-firm-block-style', esc_url( get_template_directory_uri() ).'/assets/css/blocks.css' );
...
function construction_firm_fonts_scripts() {
	$headings_font = esc_html(get_theme_mod('construction_firm_headings_text'));
	$body_font = esc_html(get_theme_mod('construction_firm_body_text'));

	if( $headings_font ) {
		wp_enqueue_style( 'construction-firm-headings-fonts', '//fonts.googleapis.com/css?family='. $headings_font );
	} else {
		wp_enqueue_style( 'construction-firm-source-sans', '//fonts.googleapis.com/css?family=Lato:400,700,400italic,700italic');
	}
	if( $body_font ) {
		wp_enqueue_style( 'construction-firm-body-fonts', '//fonts.googleapis.com/css?family='. $body_font );
	} else {
		wp_enqueue_style( 'construction-firm-source-body', '//fonts.googleapis.com/css?family=Source+Sans+Pro:400,300,400italic,700,600');
	}
}
add_action( 'wp_enqueue_scripts', 'construction_firm_fonts_scripts' );

function construction_firm_enqueue_admin_script( $hook ) {
	
	// Admin JS
	wp_enqueue_script( 'construction-firm-admin.js', get_theme_file_uri( '/assets/js/construction-firm-admin.js' ), array( 'jquery' ), true );

	wp_localize_script('construction-firm-admin.js', 'construction_firm_scripts_localize',
        array(
            'ajax_url' => esc_url(admin_url('admin-ajax.php'))
        )
    ); 
}
add_action( 'admin_enqueue_scripts', 'construction_firm_enqueue_admin_script' );

// Enqueue editor styles for Gutenberg
function construction_firm_block_editor_styles() {
	// Block styles.
	wp_enqueue_style( 'construction-firm-block-editor-style', trailingslashit( esc_url ( get_template_directory_uri() ) ) . '/assets/css/editor-blocks.css' );

	// Add custom fonts.
	wp_enqueue_style( 'construction-firm-fonts', construction_firm_fonts_url(), array(), null );
}
add_action( 'enqueue_block_editor_assets', 'construction_firm_block_editor_styles' );
